...
def insert_data_skills(cur, df, table, model_fields, excel_header):
    for index, row in df.iterrows():
        rows = ()
        number_of_columns = len(model_fields.split(',')) - 1
        for value in excel_header:
...
insert_data_skills(cur, df_colaborador_soft, "gestao_competencia_colaboradorsoftskill", "colaborador_id,soft_skill_id,score_soft", ["id_colab", "id_soft_skill"])
insert_data_skills(cur, df_colaborador_hard, "gestao_competencia_colaboradorhardskill", "colaborador_id,hard_skill_id,score_hard", ["id_colab", "id_hard_skill"])
